...
export default function Profile({ session, onBack }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-50">
      ...
      <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-emerald-400/20 to-transparent pointer-events-none" />
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 animate-fadeIn">
          ...
          {message && (
            <div className={`mt-4 p-3 rounded-lg text-sm ${
              message.includes('สำเร็จ') ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'
            }`}>
              {message}
            </div>
          )}
